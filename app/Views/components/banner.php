<!-- Banner Section -->
<section class="banner-section">
    <!-- Social Nav -->
    <ul class="social-nav">
        <li class="facebook"><a href="#"><span class="fa fa-instagram"></span></a></li>
        <li class="twitter"><a href="#"><span class="fa fa-twitter"></span></a></li>
        <li class="linkedin"><a href="#"><span class="fa fa-linkedin"></span></a></li>
    </ul>
    <div class="main-slider-carousel owl-carousel owl-theme">

        <div class="slide" style="background: url(assets/images/slider/money.jpg) no-repeat center/cover">
            <div class="auto-container">

                <!-- Content Column -->
                <div class="content-column">
                    <div class="inner-column">
                        <div class="title">Kemi Advocates</div>
                        <h1>Tax Law</h1>
                        <div class="text">The Firm completes its corporate and commercial practice with advice on
                            potential tax implications on the structuring and implementation of various transactions.
                        </div>
                        <div class="btns-box">
                            <a href="contact.html" class="theme-btn btn-style-one"><span class="txt">Get Consultation
                                    <i class="arrow flaticon-right"></i></span></a>
                        </div>
                    </div>
                </div>

            </div>
        </div>

        <div class="slide" style="background: url(assets/images/slider/real-estate.jpg) no-repeat center/cover">
            <div class="auto-container">

                <!-- Content Column -->
                <div class="content-column">
                    <div class="inner-column">
                        <div class="title">Kemi Advocates</div>
                        <h1>Real Estate <br>& Conveyance</h1>
                        <div class="text">We have considerable experience in the sale and purchase of residential and
                            commercial properties and liaising with relevant institutions/individuals for financing such
                            sales</div>
                        <div class="btns-box">
                            <a href="contact.html" class="theme-btn btn-style-one"><span class="txt">Get Consultation
                                    <i class="arrow flaticon-right"></i></span></a>
                        </div>
                    </div>
                </div>

            </div>
        </div>

        <div class="slide" style="background: url(assets/images/slider/business.jpg) no-repeat center/cover">
            <div class="auto-container">

                <!-- Content Column -->
                <div class="content-column">
                    <div class="inner-column">
                        <div class="title">Kemi Advocates</div>
                        <h1>Banking <br> & Finance</h1>
                        <div class="text">We provide a range of financial legal services for banks, financial
                            institutions and specific clients. We act on their behalf in connection with their
                            implementation of banking securities perfection (mortgages, liens) and project finance loan
                            agreements and securities.</div>
                        <div class="btns-box">
                            <a href="contact.html" class="theme-btn btn-style-one"><span class="txt">Get Consultation
                                    <i class="arrow flaticon-right"></i></span></a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
        <div class="slide" style="background: url(assets/images/slider/energy.jpg) no-repeat center/cover">
            <div class="auto-container">

                <!-- Content Column -->
                <div class="content-column">
                    <div class="inner-column">
                        <div class="title">Kemi Advocates</div>
                        <h1>Energy,Oil,Gas <br> & Mining</h1>
                        <div class="text">Kemi Advocates can facilitate our clients in energy, oil, gas and mining to
                            structure projects, acquire finance and implement projects and manage their investments
                            since we have the aforesaid expertise</div>
                        <div class="btns-box">
                            <a href="contact.html" class="theme-btn btn-style-one"><span class="txt">Get Consultation
                                    <i class="arrow flaticon-right"></i></span></a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
        <div class="slide" style="background: url(assets/images/slider/lit.jpg) no-repeat center/cover">
            <div class="auto-container">

                <!-- Content Column -->
                <div class="content-column">
                    <div class="inner-column">
                        <div class="title">Kemi Advocates</div>
                        <h1>Litigation <br> & ADR</h1>
                        <div class="text">Our Litigation and Alternative Dispute Resolution (ADR) team provides
                            competent Litigation services for our clients. Specifically, we are experienced on
                            Commercial Litigation, Corporate Law Issues.</div>
                        <div class="btns-box">
                            <a href="contact.html" class="theme-btn btn-style-one"><span class="txt">Get Consultation
                                    <i class="arrow flaticon-right"></i></span></a>
                        </div>
                    </div>
                </div>

            </div>
        </div>

    </div>

</section>
<!-- End Banner Section -->